...
void ADC3_Init(void)
{
	...
	//先初始化ADC3通道6 IO口
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_6;                                  //PF6 通道6
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AN;                                //模拟输入
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL ;                           //不带上下拉
...
/**************************************************
	...
	* @param  ch: ADCx的通道
	* @param  times: 取平均的次数
	* @return u16: 返回采集到的电压值
	* 
***************************************************/
u16 Get_Adc_Average(u8 index,u8 ch,u8 times)
{
	u32 temp_val=0;
	u8 t;
	for(t=0;t<times;t++)
	{
		temp_val+=Get_Adc(index,ch);                                            //获取对应通道的电压值
		delay_ms(5);
	}
	return temp_val/times;                                                      //返回平均电压值
} 
